 ...
 const opciones = ejercicio.opciones.map(opcion => {
  ...
  if (ejercicio.imagen) {
    agregarMensajeConImagenHTML(
      `${encabezado}<strong>${escaparHTML(ejercicio.pregunta)}</strong>`,
      ejercicio.imagen,
      "bot",
      true
    );
...
function generarEjercicioNombrePoligono() {
  const figuras = obtenerPoligonosPrincipales().filter(figura => figura.imagen);
  ...
  return {
    ...
    pregunta: "Nombre de la figura:",
    imagen: correcta.imagen,
    respuesta: correcta.nombre,
    respuestaTexto: capitalizar(correcta.nombre),
    explicacion: correcta.descripcion,
    solucion: `La figura mostrada es un ${correcta.nombre}.`,
    opciones
  };
}
...
/*
  Revisa una respuesta escrita en Matemáticas.
  Permite que el estudiante escriba:
  - 4
  - 4 lados
  - 24
  - 24 cm
...
/*
  Revisa una respuesta escrita en Matemáticas.
  Funciona para:
  - nombres de figuras: triángulo, cuadrado, hexágono
  - cantidad de lados: 4, 4 lados
  - perímetros: 24, 24 cm
*/
function revisarRespuestaGeometriaTexto(textoUsuario) {
  const ejercicio = MathGeometryState.ejercicioActual;
  ...
  let respuestaUsuario = texto;

  if (
    ejercicio.tipo === "cantidad_lados" ||
    ejercicio.tipo === "perimetro"
  ) {
    const numeroEncontrado = texto.match(/\d+/);

    if (numeroEncontrado) {
      respuestaUsuario = Number(numeroEncontrado[0]);
    }
  }

  if (ejercicio.tipo === "nombre_poligono") {
    const respuestaCorrecta = limpiarTexto(ejercicio.respuesta);

    if (texto.includes(respuestaCorrecta)) {
      respuestaUsuario = ejercicio.respuesta;
    }
  }

  revisarRespuestaGeometria(respuestaUsuario);
}
